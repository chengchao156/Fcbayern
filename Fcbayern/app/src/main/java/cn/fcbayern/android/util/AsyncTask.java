...
public abstract class AsyncTask<Params, Progress, Result> {
    ...
    public static synchronized Executor getDualThreadExecutor() {
        if (DUAL_THREAD_EXECUTOR == null || DUAL_THREAD_EXECUTOR.isShutdown()) {
            LogUtils.v(TAG, "getDualThreadExecutor(), executor ");
            DUAL_THREAD_EXECUTOR =  new ThreadPoolExecutor(CORE_POOL_SIZE, MAXIMUM_POOL_SIZE,
                    0L, TimeUnit.MILLISECONDS,
                    new LinkedBlockingQueue<Runnable>(),
                    sThreadFactory, new ThreadPoolExecutor.DiscardOldestPolicy());
        }

        return DUAL_THREAD_EXECUTOR;
    }
